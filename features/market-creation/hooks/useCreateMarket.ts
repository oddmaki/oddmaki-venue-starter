...
const MARKET_CREATED_TOPIC = '0xdd895a7ed0ebaa00381dbd1d1264310105210e93f5b57edb84d38c5401c353de';
...
export function useCreateMarket() {
  ...
  const startCreateMarket = useCallback(
    async (params: CreateMarketParams) => {
      ...
      // AC override logic
      const needsACOverride = !!params.tradingACType && params.tradingACType !== 'public';
      const needsDeployAC = needsACOverride && params.tradingACType !== 'custom';
      ...
      if (needsACOverride && params.tradingACType === 'custom') {
        acAddress = (params.tradingCustomAddress || ZERO_ADDRESS) as `0x${string}`;
      }
      ...
      steps.push({
        ...
        execute: async () => {
          ...
          if (needsMarketId) {
            ...
            if (!log || !log.topics[1]) throw new Error('MarketCreated event not found in receipt');
    ...
    [address, client, publicClient, flow, deployWhitelist, deployNFTGated, deployTokenGated],
  );
