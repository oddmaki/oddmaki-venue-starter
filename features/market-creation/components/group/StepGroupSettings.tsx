...
import type { GroupMarketFormData, TickSize } from "../../types";
...
interface StepGroupSettingsProps {
...
export function StepGroupSettings({
  formData,
  updateField,
  baseUmaReward,
}: StepGroupSettingsProps) {
  const validOutcomes = formData.outcomes.filter(
    (o) => o.name.trim() && o.question.trim(),
  ).length;
  const placeholderCap = Math.max(0, 50 - validOutcomes);
  ...
  return (
    <div style={{ display: "grid", gap: 24 }}>
      ...
      <Field
        hint={
          baseUmaReward !== null
            ? `On top of the venue's ${baseUmaReward} USDC base reward.`
      ...
      <Field
        hint={`Optional: pre-allocate empty slots that you can activate later with real outcomes. Max ${placeholderCap}.`}
      ...
      >
        <input
          ...
          style={monoInputStyle}
          type="number"
          value={formData.placeholderCount}
          onChange={(e) =>
            updateField(
              "placeholderCount",
              Math.max(
                0,
                Math.min(placeholderCap, parseInt(e.target.value) || 0),
              ),
            )
          }
        />
      ...
      >
        <input
          ...
          type="checkbox"
          onChange={(e) => updateField("activateImmediately", e.target.checked)}
        />
        <div>
          <div
            style={{
          ...
          >
            {formData.activateImmediately
              ? "Group becomes tradable in the same flow. Total markets are locked at activation."
              : "Group stays in Draft. You can add more outcomes/placeholders before activating later."}
          </div>
        </div>
      </label>
